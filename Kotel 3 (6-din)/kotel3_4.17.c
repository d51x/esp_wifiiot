...
void pageMain(){
 ...
 if(schedule){
  static p=0;
  strcpy(sc,"");
  for(u8_t j=0;j<3;j++) strcat(sc,j==p?">":"-");
  p++;
  if(p>3) p=0;
  strcat(sc," ");
 }else strcpy(sc,i%2?"   >":"    ");
...
//!	!		)		#	#
 snprintf(s,21,"%c%c %c %c%c %4sTs:%2d.%1d%c"
 ,schedule?S_SCHD1:' ',schedule?S_SCHD2:' '
 ,GPIO_ALL_GET(ESC_IO)?S_MOON:' '
 ,GPIO_ALL_GET(VENT_IO)?S_VENT1:' ',GPIO_ALL_GET(VENT_IO)?S_VENT2:' '
 ,sc,THERMO_SETPOINT(1)/10,THERMO_SETPOINT(1)%10,S_DEG);
 lcdPrint(2,s);
 
 int _temp=streetT%10;
 _temp=(_temp<0)?_temp*-1:_temp;
 snprintf(s,21,"T%c:%3d.%1d%c   T#:%2d.%1d%c",S_SUN,streetT/10,_temp%10,S_DEG,curT/10,curT%10,S_DEG);
 lcdPrint(3,s);
 i++;
}
...
void webfunc(char *pbuf){
 printKotel(pbuf);
 char s[60]="Ручное управление";
 u16_t t1=THERMO_SETPOINT(3)-THERMO_HYSTERESIS(3);
 u16_t t2=THERMO_SETPOINT(4)-THERMO_HYSTERESIS(4);
 switch(PUMP_MODE){
  case PUMP_MODE_1:snprintf(s,60,"по реле котла");break;    
  case PUMP_MODE_2:snprintf(s,60,"через %d сек после выкл котла",PUMP_OFF_TIMEOUT/1000);break;    
  case PUMP_MODE_3:snprintf(s,60,"по T-обратки #1 %d.%d° #2 %d.%d°",t1/10,t1%10,t2/10,t2%10);break;  
  default:break;  
 }
