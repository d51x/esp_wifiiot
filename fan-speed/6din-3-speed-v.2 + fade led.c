...
int mapToDuty(int duty, int period){
    return (duty * period) / 255;
}

int mapFromDuty(int duty, int period){
...
void receiveMqtt(char *topicBuf,char *dataBuf){
    ...
    if ( !strcoll(topic, "fanspeed")) {
        ...
        FAN_SPEED = m;
    } else if ( strcoll(topic, "fade") > 0) {
        char *s = (char *)os_strstr(topic, "fade");     
        char ch[3];
        strcpy(ch, s+4);
        if (ch) {
